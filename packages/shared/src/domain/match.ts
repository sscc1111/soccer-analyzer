...
export const ANALYSIS_STEP_INFO: Record<AnalysisStep, { label: string; labelJa: string }> = {
  ...
  upload_video_to_gemini: { label: "Uploading video", labelJa: "動画アップロード中" },
  ...
  detect_events_gemini: { label: "Detecting events", labelJa: "イベント検出中" },
  identify_players_gemini: { label: "Identifying players", labelJa: "選手識別中" },
  ...
  generate_tactical_insights: { label: "Generating tactical insights", labelJa: "戦術分析中" },
  generate_match_summary: { label: "Generating match summary", labelJa: "試合サマリー生成中" },
  // Consolidated analysis steps (2-call architecture)
  comprehensive_analysis: { label: "Analyzing video", labelJa: "動画分析中" },
  summary_and_tactics: { label: "Generating summary and tactics", labelJa: "サマリーと戦術分析中" },
  // Hybrid 4-call pipeline steps
  segment_and_events: { label: "Analyzing segments and events", labelJa: "セグメント・イベント分析中" },
  scenes_and_players: { label: "Analyzing scenes and players", labelJa: "シーン・選手分析中" },
