...
export type ValidatedDefaultSettings = z.infer<typeof defaultSettingsSchema>;
export type ValidatedRosterItem = z.infer<typeof rosterItemSchema>;

// Validation helper functions
export function validateMatchSettings(data: unknown) {
  return matchSettingsSchema.safeParse(data);
}

export function validateDefaultSettings(data: unknown) {
  return defaultSettingsSchema.safeParse(data);
}

// Check for duplicate jersey numbers
export function findDuplicateJerseyNumbers(
  roster: { jerseyNo: number }[]
): number[] {
